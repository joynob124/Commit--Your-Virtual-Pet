...
function initializeDatabase() {
  db.serialize(() => {
    ...
    db.run(`ALTER TABLE pets ADD COLUMN last_health_update DATETIME DEFAULT CURRENT_TIMESTAMP`, (err) => {
    ...
    db.run(`CREATE TABLE IF NOT EXISTS social_log (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      pet_id INTEGER NOT NULL,
      site TEXT NOT NULL,
...
function killPet(pet, causeOfDeath, callback) {
  const daysLived = Math.floor((Date.now() - new Date(pet.birth_date + 'Z').getTime()) / (1000 * 60 * 60 * 24));
  db.run(
    ...
    (err) => {
      ...
      db.run('UPDATE pets SET is_alive = 0, health = 0 WHERE id = ?', [pet.id], (err) => {
    ...
    (err, pet) => {
      ...
      if (decay > 0) {
        ...
        db.run(
          ...
          [newHealth, pet.id],
          (err) => {
            if (err) console.error('Error updating decayed health:', err.message);
  ...
  db.get(
    ...
    async (err, pet) => {
        ...
        db.run(
          ...
          (err) => {
            ...
            return res.json({
              ...
              newCommits: 1,
              health: newHealth,
              message: 'Simulated Commit: Code pushed! Pet fully healed to 100%! 🩹🎉'
            });
          }
        );
        return;
      ...
      try {
        ...
        const githubResponse = await fetch(`https://api.github.com/users/${pet.username}/events`, {
          headers: {
        ...
        if (!githubResponse.ok) {
          if (githubResponse.status === 404) {
            return res.status(404).json({ error: `GitHub username @${pet.username} not found.` });
          }
          return res.status(502).json({ error: 'Failed to fetch events from GitHub.' });
        }
        ...
        if (pet.last_github_event_id === latestPush.id) {
          return res.json({
            ...
            newCommits: 0,
            health: pet.health,
            message: 'No new commits since your last sync! Keep committing on GitHub! 💻'
          });
        }
        ...
        db.run(
          ...
          (err) => {
            ...
            return res.json({
              ...
              newCommits: 1,
              health: newHealth,
              message: 'New commit detected on GitHub! Pet fully healed to 100%! 🩹🎉'
            });
          }
        );
      } catch (error) {
        console.error('GitHub sync error:', error);
        return res.status(500).json({ error: 'Server error during GitHub sync' });
      }
    }
  );
});
...
app.post('/api/pets/:petId/social-penalty', (req, res) => {
  ...
  if (healthLoss === 0) {
    return res.json({ success: true, message: 'Not enough time yet to penalize.' });
  }

  db.get('SELECT * FROM pets WHERE id = ? AND is_alive = 1', [petId], (err, pet) => {
    if (err) return res.status(500).json({ error: 'Database error' });
    ...
    db.run(
      ...
      (err) => {
        ...
        db.run(
          ...
          (err) => {
            if (err) console.error('Error logging social penalty:', err.message);
          }
        );
